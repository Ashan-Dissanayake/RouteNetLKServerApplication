...
public class RosterShiftAssignmentDraftState implements RosterShiftAssignmentState{
    private static final List<String> ALLOWED = List.of("PROPOSED", "CANCELLED");

    @Override
    public void transitionTo(RosterShiftAssignment assignment, RosterShiftAssignmentStatus newStatus) {
        String newStatusName = newStatus.getName().trim().toUpperCase();
        if (!ALLOWED.contains(newStatusName)) {
            throw new InvalidStateTransitionException("Invalid transition from DRAFT to " + newStatus.getName());
        }
